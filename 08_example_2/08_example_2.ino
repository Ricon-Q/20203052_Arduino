...
float current_dist_raw; //직전 거리 저장
...
// get a distance reading from the USS
  dist_raw = USS_measure(PIN_TRIG,PIN_ECHO, current_dist_raw);

// output the read value to the serial port
  Serial.print("Min:0,");
  Serial.print("raw:");
  Serial.print(dist_raw);
  current_dist_raw = dist_raw;
  Serial.print(",");
...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    analogWrite(PIN_LED, abs(200-dist_raw)*2.55);
  }

// do something here
//  delay(50); // Assume that it takes 5ms to do something.
...
float USS_measure(int TRIG, int ECHO, float current_dist_raw)
{
  float reading;
...
//  if((reading < dist_min || reading > dist_max) && (100 < current_dist_raw  && current_dist_raw < 300)) reading = current_dist_raw; // return 0 when out of range.
//  return reading;
//  if(reading < dist_min || reading > dist_max) reading = 0.0;
//  return reading;
  if(reading < dist_min || reading > dist_max) reading = current_dist_raw;
  return reading;
