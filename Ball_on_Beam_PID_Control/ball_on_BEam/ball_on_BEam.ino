...
#define _DIST_MIN 70   //[3050] 거리의 최솟값이 100mm
...
#define _DIST_ALPHA 0.5   //[3054] 센서 보정정도
          // [3131] EMA필터값
// Servo range
#define _DUTY_MIN 900    // [3052] 서보의 최소각도를 microseconds로 표현
#define _DUTY_NEU 1550    // [3070] 레일플레이트 중립위치를 만드는 서보 duty값
#define _DUTY_MAX 2000                //[3062] 서보의 최대각도의 microseconds의 값
...
// PID parameters
#define _KP 0.5  // 최적값 3
#define _KI 1.5   // [2635] I 이득 비율
#define _KD 55.0 // [2635] D 이득 비율
#define _KI_MAX 15
...
const float coE[] = {-0.0000102, 0.0071052, -0.4492807, 103.1189299};
...
//        dist_raw = filtered_ir_distance();
        ...
        if(abs(iterm) > _KI_MAX) iterm = 0;
        iterm += _KI * error_curr;
        control = pterm + iterm + dterm; // [2635] control 값, i와 d는 현재 0
      
        // duty_target = f(duty_neutral, control)
        duty_target = _DUTY_NEU + 1.2 *control; // [3070] control 값이 다 합해서 1이 되도록 되어있다면, 중립 위치에 컨트롤 값 만큼의 비율을 더해 목표위치를 정한다.
        //duty_target = _DUTY_NEU + (control - X) * (control > X ? _DUTY_MAX - _DUTY_NEU : _DUTY_NEU - _DUTY_MIN) // [2635] NEU 에다가 위아래로 control 에 맞게 조절한 값?
